...
namespace HomieInternals {
  ...
  const uint8_t MAX_CONFIG_SETTING_SIZE = 16;
  // 6 elements at root, 9 elements at wifi (or n * 9 elements), 6 elements at mqtt, 1 element at ota, max settings elements
  const uint16_t MAX_JSON_CONFIG_ARDUINOJSON_BUFFER_SIZE = JSON_OBJECT_SIZE(6) + JSON_OBJECT_SIZE(6) + JSON_OBJECT_SIZE(9) + JSON_OBJECT_SIZE(7) + JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(MAX_CONFIG_SETTING_SIZE);
  ...
  const uint8_t MAX_WIFI_CONFIG = 5; // Max number of WiFi SSID COnfig in config file
